//+------------------------------------------------------------------+
//| File: NewYork_SwingTrend_AUDUSD_H1.mq5                            |
//| EA  : NewYork_SwingTrend_AUDUSD_H1                                |
//| Ver : 1.1.0                                                       |
//|                                                                  |
//| New York Swing Trend (AUDUSD / H1)                                |
//| - Trend: EMA(50) vs EMA(200) on closed bars                       |
//| - Entry: RSI pullback in trend direction (closed bar)             |
//| - Volatility safety: ATR(14) within [min,max] points              |
...
//|                                                                  |
//| Notes                                                             |
//| - Attach ONLY to AUDUSD H1 chart                                  |
//| - All decisions use CLOSED bars                                   |
//| - Session hours are broker/server time; adjust inputs as needed    |
//+------------------------------------------------------------------+
#property strict
...
// ------------------------- Identity / Tracking ---------------------
input string InpEaId          = "NY_ST_AUDUSD_H1";
input string InpEaName        = "NewYork_SwingTrend_AUDUSD_H1";
...
input double InpRsiBuyMax              = 45.0;     // uptrend pullback
input double InpRsiSellMin             = 55.0;     // downtrend pullback

input int    InpAtrPeriod              = 14;
input int    InpAtrMinPoints           = 70;
input int    InpAtrMaxPoints           = 280;

input double InpAtrSLMult              = 2.6;
...
bool GetSignals(bool &buySig, bool &sellSig, double &atrPtsOut)
{
   ...
   if(!upTrend && !downTrend) { g_diag.block_nobias++; return true; }

   if(upTrend && rsi1 <= InpRsiBuyMax)     buySig  = true;
   if(downTrend && rsi1 >= InpRsiSellMin)  sellSig = true;
...
{
   if(_Symbol != "AUDUSD")
      Print("Warning: This EA is intended for AUDUSD. Current symbol=", _Symbol);
